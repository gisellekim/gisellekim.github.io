	...
	// resize info
		$(window).resize(function() {
			if ($(window).width()<813){
				$('.bedroom_cabinet').removeClass('below');
				$('.restroom_basket').removeClass('below');
				$('.restroom_storage').removeClass('below');
				...
				$('.bedroom_cabinet').addClass('below');
				$('.restroom_storage').addClass('below');
				$('.restroom_basket').addClass('below');
	...
	// mobile menu
		$('.menu').click(function(){
			$(this).toggleClass('open');
			  ...
			  if ( $(this).parent().find('>.product').css('visibility') == 'hidden' ){
				$(this).parent().find('>.product').css('visibility','visible');
			  } else{
				$(this).parent().find('>.product').css('visibility','hidden');
			  }
	...
	// product info
		$('.contents .product').hover(function(){
